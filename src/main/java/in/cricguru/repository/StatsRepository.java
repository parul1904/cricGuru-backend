...
public interface StatsRepository extends JpaRepository<Stats, Long> {
    ...
    @Query(value = """
            ...
            WHERE sq.season_id = 2\s
            AND sq.team_id IN (:team1Id, :team2Id)
            ),
            DreamTeamRanks AS (SELECT match_no, player_id,\s
            CASE\s
              WHEN ROW_NUMBER() OVER (PARTITION BY match_no ORDER BY total_point_dream11_new_system DESC) <= 11\s
              THEN true\s
              ELSE false\s
              END as is_in_dream_team
            FROM match_stats),
            LastMatchStats AS (SELECT player_id, total_point_dream11_new_system as last_match_points, match_no as last_match_no
            FROM (SELECT player_id, total_point_dream11_new_system, match_no, ROW_NUMBER() OVER (PARTITION BY player_id ORDER BY match_no DESC) as rn
            FROM match_stats) ranked WHERE rn = 1),
            PlayerMatchStats AS (SELECT  ms.player_id, ms.match_no, m.match_date, t1.team_name as team1_name, t1.team_logo_url as team1_logo,
            t2.team_name as team2_name, t2.team_logo_url as team2_logo, ms.total_point_dream11_new_system as points, ms.runs_scored, ms.ball_faced,
            ...
            FROM CurrentSquadPlayers csp LEFT JOIN PlayerMatchStats pms ON csp.player_id = pms.player_id AND pms.match_rank <= 5
            ...
            """, nativeQuery = true)
    List<Object[]> getPlayerPerformanceStats(@Param("team1Id") Long team1Id, @Param("team2Id") Long team2Id);
